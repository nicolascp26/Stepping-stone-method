...
public class TransportationProblem2 {
    ...
    static void init(String filename) throws FileNotFoundException {
        ...
        // Lee el archivo que contiene los datos
        try (Scanner sc = new Scanner(new File(filename))) {
            ...
            // Se crea la matriz costos y una matriz de objetos Cargamento
    ...
    static Shipment[] getClosedPath(Shipment s) {
        ...
        // remueve (y sigue removiendo) elementos que no tienen un
        // vecino Horizontal Y vertical
        ...
        // coloca los elementos restantes en un orden de mas-menos
    ...
    static void fixDegenerateCase() {
        ...
        // Verifica que se cumpla la LEY DE LA NO DEGENERACION
    ...
    static void printDetailedResult() {
        System.out.printf("SOLUCION OPTIMA: %n%n");
        double totalCosts = 0;
        for (int r = 0; r < supply.length; r++) {
            for (int c = 0; c < demand.length; c++) {
        ...
        for (int r = 0; r < supply.length; r++) {
            for (int c = 0; c < demand.length; c++) {
                ...
                Shipment s = matrix[r][c];
                if (s != null && s.r == r && s.c == c) {
                    System.out.printf("Se debe enviar "+s.quantity+" unidades desde P"+(r+1)+" hasta A"+(c+1)+"\n");   
                }
            }
        }
        System.out.printf("%n Para tener un COSTO MINIMO de: %s%n%n", totalCosts);
    }

    public static void main(String[] args) throws Exception {

        String filename = "C:/Users/Nicolas Carreno/Documents/Universidad/Optimizacion/input1.txt";
        init(filename);
        northWestCornerRule();
        steppingStone();
        printDetailedResult();
